...
for ndar_input in ndar_file_list:
	...
	if run_count < run_limit:
		...
		intermediate_file = '/'.join(ndar_input['targetfile'].split('/')[0:-2]) + '/converted/ndar_' + ndar_input['targetfile'].split('/')[-1]

		print 'Intermediate Output= ' + intermediate_file
		...
		# "Variable / Field Name", 							-> ndar::ElementName
			...
			redcap_item.append(ndar_item['element_name'])
		...
		# "Question Number (surveys only)"					-> undefined, custom
			...
			single_item.append(redcap_item)
		
	f = open(intermediate_file, 'wt')
